...
export default function Products({ items }) {
	return (
		<section className="flex flex-col justify-center">
			<div ref={parent} className="grid grid-cols-4 gap-x-2.5 gap-y-5 lg:grid-cols-2">
				{items.map((item, index) => (
					<a
						...
						<h6 className="font-semibold mb-1.5 leading-[100%] text-[18px] pl-2.5 md:text-[16px] md:leading-[120%]">{item.title}</h6>
			...
			<button className="uppercase hover:bg-secondary_dark bg-dark text-white rounded-lg font-inter font-semibold text-[24px] mt-10 px-[50px] py-4 mx-auto leading-[125%] md:text-[18px] md:px-5 md:w-full">Загрузить больше</button>
		</section>
	);
}
